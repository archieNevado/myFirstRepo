 ...
 */
public class ShopProductSearchResultDocument extends AbstractOCDocument {

  private static final String CATEGORY_ID_REFINE = "cgid";
  ...
  public List<ProductSearchRefinementDocument> getRefinements() {
    return refinements.stream()
      //exclude the category refinement
      .filter(refinementDocument -> !refinementDocument.getAttributeId().equals(CATEGORY_ID_REFINE))
      //exclude refinements with no values as they are irrelevant and not visible on the shop front.
      .filter(refinementDocument -> !refinementDocument.getChildFacets().isEmpty())
      .collect(Collectors.toList());
  }

  public void setRefinements(List<ProductSearchRefinementDocument> refinements) {
