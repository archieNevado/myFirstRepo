...
<#if loginAction?has_content && logoutAction?has_content && profileAction?has_content>
  ...
  <a href="${cm.getLink(self.profileAction!cm.UNDEFINED)}"
     title=""<@preview.metadata data=[profileAction.content, "properties.id"] />><@cm.message "userDetails_title" /></a>
  <#else>
  <a data-href="${cm.getLink(self, {"next": "$nextUrl$", "absolute": true, "scheme": lc.getSecureScheme()})}"<@preview.metadata data=[loginAction.content, "properties.id"] />><@cm.message "login_title" /></a>
  </#if>
</#if>
