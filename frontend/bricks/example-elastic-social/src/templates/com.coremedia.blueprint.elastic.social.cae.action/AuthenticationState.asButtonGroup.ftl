...
<#if loginAction?has_content && logoutAction?has_content && profileAction?has_content>
  ...
  <#else>
    <div class="cm-button-group cm-button-group--default">
      <@components.button href="#" text=cm.getMessage("login_title") iconClass="icon-profile-locked" attr={"metadata": [loginAction.content, "properties.id"], "classes": ["cm-button-group__button"], "data-href": cm.getLink(self, {"next": "$nextUrl$", "absolute": true})} />
    </div>
  </#if>
</#if>
