...
/**
 *  CoreMedia Blueprint Javascript Framework Extension for LiveContext
 */
coremedia.blueprint.livecontext = function (module) {
  "use strict";
   ...
   */
  var marketingSpotItemSelector = ".cm-collection--marketingspot .cm-teasable--text";
...
coremedia.blueprint.$(function () {
  ...
  coremedia.blueprint.nodeDecorationService.addNodeDecoratorBySelector(".cm-product-assets__slideshow", function ($target) {
    ...
    // can be calculated
    $target.on(coremedia.blueprint.livecontext.PDP_ASSET_READY_EVENT, function () {
    ...
    $target.findAndSelf(selector).each(function () {
      ...
      if (config.container !== undefined && config.item !== undefined) {
        ...
        var cycleConfig = {
          ...
          speed: (coremedia.blueprint.deviceDetector.getLastDevice().type == coremedia.blueprint.livecontext.DEVICE_DESKTOP) ? 1000 : 600,
          pauseOnHover: true,
          fx: "scrollHorz",
          swipe: true,
  ...
  coremedia.blueprint.nodeDecorationService.addNodeDecoratorBySelector(".cm-product-assets", function ($target) {
    ...
    $.when(slideshowDeferrer, carouselDeferrer).done(function() {
      $(document).trigger(coremedia.blueprint.basic.EVENT_LAYOUT_CHANGED);
      $slideshow.trigger(coremedia.blueprint.livecontext.PDP_ASSET_READY_EVENT);
    });
    ...
    $target.findAndSelf(selector).each(function () {
      ...
      if (config.cart !== undefined) {
        $control.find(config.cart).on("cartUpdated", function () {
          coremedia.blueprint.livecontext.updateCartControl($control);
        });
      }
      coremedia.blueprint.livecontext.updateCartControl($control);
    });
  ...
  var updateTimer = 0;
  coremedia.blueprint.nodeDecorationService.addNodeDecoratorBySelector(".cm-richtext img", function ($target) {
  ...
  var layout = function () {
    ...
    // only on desktop
    if (coremedia.blueprint.deviceDetector.getLastDevice().type == coremedia.blueprint.livecontext.DEVICE_DESKTOP) {
      coremedia.blueprint.livecontext.setMegaMenuItemsWidth();
    }
    // on desktop and tablet
    if (coremedia.blueprint.deviceDetector.getLastDevice().type == coremedia.blueprint.livecontext.DEVICE_DESKTOP
            || coremedia.blueprint.deviceDetector.getLastDevice().type == coremedia.blueprint.livecontext.DEVICE_TABLET) {
      coremedia.blueprint.livecontext.setMarketingSpotItemsHeight();
    }
