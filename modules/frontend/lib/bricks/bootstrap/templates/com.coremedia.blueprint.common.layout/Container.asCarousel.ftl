      ...
      <#list items as item>
        ...
        <#if item_index == 0>
          ...
          <div class="${itemCssClass!"item"}">
            <@cm.include self=item view=viewItems!"asTeaser" params={"index": index, "viewItemCssClass": viewItemCssClass} + carouselItemParams/>
          </div>
      </#list>
    </div>

<#-- Controls -->
  <#if displayControls!true && (items?size > 1) >
      <div class="cm-carousel__control ${clsModifier}">
          <a class="left cm-carousel-control carousel-control" href="#${carouselId}" role="button" data-slide="prev">
              <span class="glyphicon glyphicon-${controlIcon!"chevron"}-left" aria-hidden="true"></span>
              <span class="sr-only">${bp.getMessage("button_previous")}</span>
          </a>
        <#if displayPagination!false>
          ...
          <a class="right cm-carousel-control carousel-control" href="#${carouselId}" role="button" data-slide="next">
              <span class="glyphicon glyphicon-${controlIcon!"chevron"}-right" aria-hidden="true"></span>
