  ...
  <div class="row">
    <div class="cm-form cm-form--resetpassword well col-xs-12 col-md-6 col-md-push-3"<@preview.metadata data=[self.action.content!"", "properties.id"]/>>
      ...
      <form method="post" data-cm-form--reset="">
        ...
        <@spring.formInput path="bpPasswordReset.password" attributes='class="form-control" placeholder="${cm.getMessage("confirmPasswordReset_password_label")}" required' fieldType="password"/>
        ...
        <@spring.formInput path="bpPasswordReset.confirmPassword" attributes='class="form-control" placeholder="${cm.getMessage("confirmPasswordReset_confirmPassword_label")}" required' fieldType="password"/>
        <#if spring.status.error><span class="help-block">${spring.status.getErrorMessagesAsString("\n")}</span></#if>
        </div>
