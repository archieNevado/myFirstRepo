    ...
    <div class="cm-form cm-form--reset well col-xs-12 col-md-6 col-md-push-3"<@preview.metadata data=[self.action.content!"", "properties.id"]/>>
      ...
      <form method="post" data-cm-form--forgot="">
        ...
        <div class="form-group">
          ...
          <@spring.formInput path="bpPasswordReset.emailAddress" attributes='class="form-control" placeholder="${cm.getMessage("passwordReset_email_label")}" required' fieldType="text"/>
        </div>

        <div class="form-group text-right">
          <@components.button text=cm.getMessage("passwordReset_button") attr={"type": "submit", "classes": ["btn","btn-primary"]} />
        </div>
      </form>
    </div>
  </div>
