...
<div class="cm-review" data-cm-review-id="${self.id}" itemscope="itemscope" itemtype="http://data-vocabulary.org/Review" <@cm.metadata self/>>
  ...
  <#if ["undecided"]?seq_contains(reviewView)>
    <@bp.notification type="info" text=bp.getMessage(es.messageKeys.REVIEW_APPROVAL_UNDECIDED) dismissable=false <#--iconClass="glyphicon glyphicon-info-sign"--> additionalClasses=["cm-review__notification"] attr={"data-cm-notification": '{"path": ""}', "data-cm-contribution-notification-type": "UNDECIDED"} />
  <#elseif ["rejected"]?seq_contains(reviewView)>
    <@bp.notification type="warning" text=bp.getMessage(es.messageKeys.REVIEW_APPROVAL_REJECTED) dismissable=false <#--iconClass="glyphicon glyphicon-exclamation-sign"--> additionalClasses=["cm-review__notification"] attr={"data-cm-notification": '{"path": ""}', "data-cm-contribution-notification-type": "REJECTED"} />
  <#elseif ["deleted"]?seq_contains(reviewView)>
  <#-- not in use -->
    <@bp.notification type="info" text=bp.getMessage(es.messageKeys.REVIEW_DELETED) dismissable=false <#--iconClass="glyphicon glyphicon-info-sign"--> additionalClasses=["cm-review__notification"] attr={"data-cm-notification": '{"path": ""}'} />
  <#else>
