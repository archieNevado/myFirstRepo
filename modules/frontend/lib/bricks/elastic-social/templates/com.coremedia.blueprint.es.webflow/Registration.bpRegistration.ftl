...
<div class="container">
  <div class="row">
    <div class="cm-form cm-form--registration well col-xs-12 col-md-6 col-md-push-3"<@cm.metadata data=[registrationAction.content!"", "properties.id"]/>>
      ...
      <form method="post" enctype="multipart/form-data" data-cm-form--registration=""<@cm.unescape (elasticSocialConfiguration.recaptchaForRegistrationRequired!false)?then(' onsubmit="substituteRecaptchaBindElement();"','') />>
        ...
        <div class="form-group<#if spring.status.error> has-error</#if>">
          <div class="checkbox">
            <label>
              ...
              <@cm.unescape text?replace("#privacyPolicy#", privacyPolicy)?replace("#termsOfUse#", termsOfUse) />
            </label>
          </div>
          <#if spring.status.error><span class="help-block">${spring.status.getErrorMessagesAsString("\n")}</span></#if>
        </div>
