...
import {
  EVENT_VIDEO_ENDED,
  EVENT_VIDEO_START,
  EVENT_VIDEO_TIME_UPDATED,
  start,
} from "./video";

const flashFallback = require("mediaelement/build/mediaelement-flash-video.swf");
...
nodeDecorationService.addNodeDecorator(function($target) {
  ...
  findAndSelf($target, selector).each(function() {
    ...
    const me = new MediaElement(this, {
      plugins: ["flash"],
      ...
      success: function(mediaElement) {
        mediaElement.addEventListener(
          ...
          function() {
            logger.log(
              "Video found with duration of " + mediaElement.duration + "ms"
            );
        ...
        mediaElement.addEventListener(
          "timeupdate",
          function(e) {
            let currentTime = e.detail.target.currentTime;
    ...
    $window.on("message", function(e) {
      ...
      if (data["player_id"] == $video.attr("id")) {
        switch (data.event) {
          ...
          case "playProgress":
            $video.trigger(EVENT_VIDEO_TIME_UPDATED, {
              position: Math.floor(data.data.seconds) * 1000,
            });
        }
      }
    });
