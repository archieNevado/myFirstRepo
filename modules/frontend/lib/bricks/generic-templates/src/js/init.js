    ...
    callbacks: {
      elementParse: function(item) {
        if (item.src.indexOf("resource/blob") > -1) {
          ...
          item.type = "inline";
          item.src =
            '<div class="cm-popup--scaler"><button title="Close (Esc)" type="button" class="mfp-close">×</button>' +
            '<video src="' +
            url +
            '" class="cm-video cm-video--html5" poster="" autoplay="autoplay" controls="controls"></video>' +
            "</div>";
        }
      },
    },
