...
<a id="${idPrefix}categoryLink_${uniqueId}" href="${link}"<#if target?has_content> target="${target?no_esc}"</#if> aria-label="${self.teaserTitle}" class="menuLink" role="menuitem" tabindex="-1"<@cm.metadata self.content/>>${self.teaserTitle}</a>
