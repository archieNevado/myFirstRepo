...
<div class="cm-container ${additionalClass}" <@cm.metadata data=[bp.getContainerMetadata(self),bp.getPlacementHighlightingMetaData(self)!""] />>
  <#if showHeadline && self.teaserTitle?has_content>
    <h2 class="cm-container__headline" <@cm.metadata data=[self.content,"properties.teaserTitle"]/>><span>${self.teaserTitle}</span></h2>
  </#if>
  <#if (numberOfItems > 0)>
    <div class="row-grid cm-flex-row <#if center>cm-flex-row--center</#if>" <#if self.content?has_content><@cm.metadata data=[self.content,"properties.items"]/></#if>>
      <#list items as item>
