...
<div id="${pagePrefixId}" class="cm-pagination__block" data-cm-page="${pageNumber}">
  <@cm.include self=bp.getContainer(self.items) view="asGrid" params={
...
<#if (nextPageNumber < numberOfPages)>
  <button disabled class="cm-pagination__more" data-cm-pagination-page="${cm.getLink(self, {"pageNum": nextPageNumber})}">
