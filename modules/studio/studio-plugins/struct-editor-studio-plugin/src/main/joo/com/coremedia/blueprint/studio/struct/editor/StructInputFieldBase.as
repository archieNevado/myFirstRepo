...
public class StructInputFieldBase extends Panel {
  ...
  public function StructInputFieldBase(config:structInputField = null) {
  ...
  private function initFields():void {
    ...
    var type:int = activeModel.getType();
      ...
      if (type === ElementConstants.ELEMENT_INT_PROPERTY) {
          fieldContainer.add(createTextField(ElementConstants.NAME_PROPERTY,false));
          fieldContainer.add(createNumberField(ElementConstants.VALUE_PROPERTY));
      }
      else if (type === ElementConstants.ELEMENT_STRING_PROPERTY ) {
          fieldContainer.add(createTextField(ElementConstants.NAME_PROPERTY,false));
          fieldContainer.add(createTextAreaField(ElementConstants.VALUE_PROPERTY));
      }
      else if(type === ElementConstants.ELEMENT_STRUCT_PROPERTY) {
      fieldContainer.add(createTextField(ElementConstants.NAME_PROPERTY,false));
    }
    else if(type === ElementConstants.ELEMENT_BOOLEAN_PROPERTY) {
      fieldContainer.add(createTextField(ElementConstants.NAME_PROPERTY,false));
      if(structHandler.getModus() == StructHandler.MODUS_FORMATTED) {
        fieldContainer.add(createBooleanField(ElementConstants.VALUE_PROPERTY));
      }
      else {
        fieldContainer.add(createTextAreaField(ElementConstants.VALUE_PROPERTY));
      }
    }
    else if(type === ElementConstants.ELEMENT_LINK_PROPERTY) {
      fieldContainer.add(createTextField(ElementConstants.NAME_PROPERTY,false));
      ...
      if(structHandler.getModus() == StructHandler.MODUS_FORMATTED && (!activeModel.getHRef() || isCapContentId(activeModel.getHRef()))) {
        linkEditor = createLinkField(ElementConstants.HREF_PROPERTY,getHrefValueExpression(), linkTypeVE);
        fieldContainer.add(linkEditor);
      } else {
        fieldContainer.add(createTextField(ElementConstants.HREF_PROPERTY, true, 'href'));
      }
      ...
      if(structHandler.getModus() == StructHandler.MODUS_FORMATTED  && (!activeModel.getLinkType() ||isCapContentTypeId(activeModel.getLinkType()))) {
        fieldContainer.add(createContentTypeField(ElementConstants.LINK_TYPE_PROPERTY,linkTypeVE, linkEditor));
      } else {
        fieldContainer.add(createTextField(ElementConstants.LINK_TYPE_PROPERTY,false));
      }
    }
    else if (type == ElementConstants.ELEMENT_STRING) {
      fieldContainer.add(createTextAreaField(ElementConstants.VALUE_PROPERTY));
    }
    else if(type == ElementConstants.ELEMENT_BOOLEAN) {
      if(structHandler.getModus() == StructHandler.MODUS_FORMATTED) {
        fieldContainer.add(createBooleanField(ElementConstants.VALUE_PROPERTY));
      }
      else {
    ...
    else if (type === ElementConstants.ELEMENT_INT) {
      fieldContainer.add(createNumberField(ElementConstants.VALUE_PROPERTY));
    }
    else if(type === ElementConstants.ELEMENT_LINK) {
      if(structHandler.getModus() == StructHandler.MODUS_FORMATTED && (!activeModel.getHRef() || isCapContentId(activeModel.getHRef()))) {
        linkTypeVE = ValueExpressionFactory.create(ElementConstants.LINK_TYPE_PROPERTY, beanFactory.createLocalBean());
        ...
        fieldContainer.add(createLinkField(ElementConstants.HREF_PROPERTY,getHrefValueExpression(), linkTypeVE));
      } else {
        fieldContainer.add(createTextField(ElementConstants.HREF_PROPERTY, true, 'href'));
      }
    }
    else if (type === ElementConstants.ELEMENT_LINK_LIST_PROPERTY) {
      fieldContainer.add(createTextField(ElementConstants.NAME_PROPERTY,false));
//      fieldContainer.add(createMinMaxField());
      if(structHandler.getModus() == StructHandler.MODUS_FORMATTED) {
        fieldContainer.add(createContentTypeField(ElementConstants.LINK_TYPE_PROPERTY,getLinkTypeValueExpression(), null));
      } else {
        fieldContainer.add(createTextField(ElementConstants.LINK_TYPE_PROPERTY,false));
      }
    }
    else if (type === ElementConstants.ELEMENT_BOOLEAN_LIST_PROPERTY ||
...
//      fieldContainer.add(lnew Label(label({text:Blueprint_properties.INSTANCE.Struct_no_attributes})));
  ...
  private function getLinkTypeValueExpression():ValueExpression {
    var ve:ValueExpression = ValueExpressionFactory.create(ElementConstants.LINK_TYPE_PROPERTY, beanFactory.createLocalBean());
  ...
  private function linkTypeChanged(ve:ValueExpression):void {
    var ltExpression:ValueExpression = ValueExpressionFactory.create(ElementConstants.LINK_TYPE_PROPERTY, activeModel);
  ...
  private function getHrefValueExpression():ValueExpression {
    var ve:ValueExpression = ValueExpressionFactory.create(ElementConstants.HREF_PROPERTY, beanFactory.createLocalBean());
  ...
  private function refChanged(ve:ValueExpression):void {
    var refExpression:ValueExpression = ValueExpressionFactory.create(ElementConstants.HREF_PROPERTY, activeModel);
  ...
  private function createMinMaxField():MinMaxEditor {
    var max:ValueExpression = ValueExpressionFactory.create(ElementConstants.MAX_PROPERTY, activeModel);
    max.addChangeListener(inputChanged);
    var min:ValueExpression = ValueExpressionFactory.create(ElementConstants.MIN_PROPERTY, activeModel);
