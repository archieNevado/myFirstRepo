...
public class StructHandler {
  ...
  public function getEnabledTypes(node:TreeNode):Array {
    ...
    if (type == ElementConstants.ELEMENT_INT_PROPERTY ||
            type == ElementConstants.ELEMENT_STRING_PROPERTY ||
            type == ElementConstants.ELEMENT_BOOLEAN_PROPERTY ||
            type == ElementConstants.ELEMENT_STRING_PROPERTY) {
    }
    else if (type == ElementConstants.ELEMENT_STRUCT_PROPERTY) {
      //allow only one child struct
      if (node.childNodes.length === 0) {
        types.push(ElementConstants.ELEMENT_STRUCT);
      }
    }
    else if (type == ElementConstants.ELEMENT_STRUCT || type == ElementConstants.ELEMENT_ROOT) { //we are working on structs so enable all property elements
      types.push(ElementConstants.ELEMENT_BOOLEAN_PROPERTY);
      types.push(ElementConstants.ELEMENT_BOOLEAN_LIST_PROPERTY);
      types.push(ElementConstants.ELEMENT_INT_PROPERTY);
      types.push(ElementConstants.ELEMENT_INT_LIST_PROPERTY);
      types.push(ElementConstants.ELEMENT_STRING_PROPERTY);
      types.push(ElementConstants.ELEMENT_STRING_LIST_PROPERTY);
      types.push(ElementConstants.ELEMENT_LINK_PROPERTY);
      types.push(ElementConstants.ELEMENT_LINK_LIST_PROPERTY);
      types.push(ElementConstants.ELEMENT_STRUCT_PROPERTY);
      types.push(ElementConstants.ELEMENT_STRUCT_LIST_PROPERTY);
    }
    else if (type == ElementConstants.ELEMENT_BOOLEAN_LIST_PROPERTY) {
      types.push(ElementConstants.ELEMENT_BOOLEAN);
    }
    else if (type == ElementConstants.ELEMENT_STRING_LIST_PROPERTY) {
      types.push(ElementConstants.ELEMENT_STRING);
    }
    else if (type == ElementConstants.ELEMENT_INT_LIST_PROPERTY) {
      types.push(ElementConstants.ELEMENT_INT);
    }
    else if (type == ElementConstants.ELEMENT_STRUCT_LIST_PROPERTY) {
      types.push(ElementConstants.ELEMENT_STRUCT);
    }
    else if (type == ElementConstants.ELEMENT_LINK_LIST_PROPERTY) {
      types.push(ElementConstants.ELEMENT_LINK);
    }
    return types;
  }
  ...
  public function getRootNode():AsyncTreeNode {
    if (!rootNode) {
      var model:ElementModel = new ElementModel(ElementConstants.ELEMENT_ROOT);
  ...
  private function getTypeForNode(node:*):int {
    ...
    for (var i:int = 0; i < ElementConstants.NAMES.length; i++) {
      if (ElementConstants.NAMES[i] == name) {
        return i;
      }
    }
    return -1;
  ...
  public function copyNode(node:TreeNode):TreeNode {
    ...
    if (clonedModel.getName()) {
      ...
      clonedModel.set(ElementConstants.NAME_PROPERTY, name);
    }
  ...
  public function isValid():Boolean {
    var xml:String = '<' + ElementConstants.NAMES[0] + '/>';
    var document:* = XMLUtil.parseXML(xml);
    if (document) {
