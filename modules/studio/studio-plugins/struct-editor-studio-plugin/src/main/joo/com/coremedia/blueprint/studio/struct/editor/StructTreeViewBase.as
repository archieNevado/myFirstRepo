...
public class StructTreeViewBase extends TreePanel {
  ...
  public function StructTreeViewBase(config:structTreeView = null) {
    super(config);
  ...
  private function initTree():void {
    ...
    for (var i:int = 0; i < ElementConstants.TYPES.length; i++) {
      var type:int = ElementConstants.TYPES[i];

      var bindDisablePluginCfg:bindDisablePlugin = new bindDisablePlugin();
      bindDisablePluginCfg.forceReadOnlyValueExpression = forceReadOnlyValueExpression;
      bindDisablePluginCfg.bindTo = bindTo;

      var tButton:structToolbarButton = new structToolbarButton();
      tButton.nodeType = type;
      tButton['visible'] = false;
      tButton.structHandler = handler;
      tButton.selectedNodeExpression = selectedNodeExpression;
      tButton.plugins = [
        bindDisablePluginCfg
      ];
  ...
  protected function nodeUpTransformer(node:TreeNode):Boolean {
    if (node && handler.getData(node).getType() !== ElementConstants.ELEMENT_ROOT) {
  ...
  private function hideAll():void {
    for (var i:int = 0; i < ElementConstants.TYPES.length; i++) {
      getButton4Type(ElementConstants.TYPES[i]).setVisible(false);
    }
  }
  ...
  protected function deleteButtonTransformer(value:TreeNode):Boolean {
    if (value &&
            handler.getData(value).getType() !== ElementConstants.ELEMENT_ROOT) {
      return false;
    }
    return true;
  }
  ...
  protected function copyButtonTransformer(value:TreeNode):Boolean {
    if (value &&
        handler.getData(value).getType() !== ElementConstants.ELEMENT_ROOT &&
        handler.getData(value.parentNode as TreeNode).getType() !== ElementConstants.ELEMENT_STRUCT_PROPERTY) { //struct has XSD min/max of 0/1
      return false;
    }
    return true;
  }
