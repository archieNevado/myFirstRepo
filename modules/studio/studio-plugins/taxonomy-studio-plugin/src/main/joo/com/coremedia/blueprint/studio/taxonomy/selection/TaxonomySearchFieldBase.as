...
public class TaxonomySearchFieldBase extends ComboBox {
  ...
  public function TaxonomySearchFieldBase(config:taxonomySearchField = null) {
    ...
    var jsonStoreCfg:jsonstore = new jsonstore();
    jsonStoreCfg.autoLoad = false;
    jsonStoreCfg.autoDestroy = true;

    var httpProxyCfg:httpproxy = new httpproxy();
    connection(httpProxyCfg).url = 'api/taxonomies/find?' + getTaxonomyIdParam() + getSiteParam();
    connection(httpProxyCfg).method = 'GET';
    httpProxyCfg['failure'] = function (response:XMLHttpRequest):void {
            Logger.info('Taxonomy search request failed:' + response.responseText);
    };

    jsonStoreCfg.proxy = new HttpProxy(httpProxyCfg);
    jsonreader(jsonStoreCfg).root = NODES;
    jsonreader(jsonStoreCfg).idProperty = 'text';
    jsonreader(jsonStoreCfg).totalProperty = 'size';

    var suggestionCountFieldCfg:datafield = new datafield();
    suggestionCountFieldCfg.name = SUGGESTION_COUNT;
    suggestionCountFieldCfg.type = 'int';

    var htmlDataFieldCfg:datafield = new datafield();
    htmlDataFieldCfg.name = TaxonomyNode.PROPERTY_HTML;
    htmlDataFieldCfg.mapping = renderHTML;

    jsonStoreCfg.fields = [
      suggestionCountFieldCfg,
      TaxonomyNode.PROPERTY_REF,
      TaxonomyNode.PROPERTY_TAXONOMY_ID,
      TaxonomyNode.PROPERTY_NAME,
      htmlDataFieldCfg,
      TaxonomyNode.PROPERTY_PATH
    ];

    var superConfig:taxonomySearchField = new taxonomySearchField();
    superConfig.store = new JsonStore(jsonStoreCfg);
    superConfig.displayField = TaxonomyNode.PROPERTY_NAME;
    superConfig.tpl = autoSuggestResultTpl;
    ...
    TabChangePlugin.getDocumentTabChangeExpression().addChangeListener(blurTextField);
    TabChangePlugin.getWorkAreaTabChangeExpression().addChangeListener(blurTextField);
  }
  ...
  override protected function onDestroy():void {
    ...
    super.onDestroy();
    TabChangePlugin.getDocumentTabChangeExpression().removeChangeListener(blurTextField);
    TabChangePlugin.getWorkAreaTabChangeExpression().removeChangeListener(blurTextField);
  }
}
}
