...
public class TaxonomyExplorerPanelBase extends Panel {
   ...
   */
  protected function createChildNode(b:Button, e:IEventObject):void {
    setBusy(true);
   ...
   */
  protected function deleteNode(b:Button, e:IEventObject):void {
    ...
    if (node && !node.isRoot()) {
      ...
      if (content) {//null after content deletion
        ...
        content.invalidate(function ():void {
          ValueExpressionFactory.create(TaxonomyStudioPluginSettings_properties.INSTANCE.taxonomy_display_property, content).addChangeListener(selectedTaxonomyNameChanged);
...
//    });
  ...
  public function setBusy(b:Boolean):void {
    ...
    if (view) {
      ...
      var dfd:Container = Container(view.find('itemId', 'documentFormDispatcher')[0]);
      var spf:Array = dfd.findByType('com.coremedia.cms.editor.sdk.config.stringPropertyField');
