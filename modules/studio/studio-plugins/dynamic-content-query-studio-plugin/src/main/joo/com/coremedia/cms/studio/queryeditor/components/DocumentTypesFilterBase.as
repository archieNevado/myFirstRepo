...
public class DocumentTypesFilterBase extends Panel {
  ...
  override protected function afterRender():void {
    ...
    Ext.each(this.getTypeCheckboxes().getRange(), function(checkboxx:Checkbox){
    ...
    if((getLocalModel().get(SELECTED_TYPES) as Array).length === 0) {
  ...
  private function renderTypes():void {
    ...
    Ext.each(documentTypes, function(documentType:String){
      var checkboxCfg:checkbox = new checkbox();
      checkboxCfg.boxLabel = typeNameToString(documentType);
      checkboxCfg.name = documentType;
      checkboxCfg.checked = true;
      checkboxCfg.ctCls = BooleanPropertyField.BOOLEAN_PROPERTY_FIELD_CSS;
      addMethod.call(typeCheckboxes, ComponentMgr.create(checkboxCfg));
    }, null);
    this.doLayout();
  }
  ...
  private function getTypeCheckboxes():MixedCollection {
    return MixedCollection(this.getTypeCheckboxesParent().items); // Wrong type in Ext AS API!
  }
