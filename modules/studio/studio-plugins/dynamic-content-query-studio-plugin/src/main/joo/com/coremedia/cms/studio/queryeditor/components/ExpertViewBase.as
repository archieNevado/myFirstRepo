...
public class ExpertViewBase extends Container {
  ...
  private function renderDocumentTypesTextField():void {
    ...
    var queryChunkFieldCfg:queryChunkField = new queryChunkField();
    queryChunkFieldCfg.bindTo = bindTo;
    queryChunkFieldCfg.defaultExpertExpression = documentTypesDefaultLocal;
    queryChunkFieldCfg.remoteExpertExpression = getExpertRemote(documentTypesRemote, documentTypesPropertyName);
    queryChunkFieldCfg.fieldLabel = QueryEditor_properties.INSTANCE.DCQE_label_selection_of_document_types;
    add(queryChunkFieldCfg);
  }
  ...
  private function renderConditionTextFields():void {
    ...
    allFilterQueryPropertiesNames.forEach(function(conditionPropertyName:String){
      ...
      var queryChunkFieldCfg:queryChunkField = new queryChunkField();
      queryChunkFieldCfg.bindTo = bindTo;
      queryChunkFieldCfg.defaultExpertExpression = conditionDefaultLocal;
      queryChunkFieldCfg.remoteExpertExpression = getExpertRemote(conditionRemote, conditionPropertyName);
      queryChunkFieldCfg.fieldLabel = ContentQueryEditorBase.getConditionTitle(conditionPropertyName);
      add(queryChunkFieldCfg);
    });
  }
  ...
  private function renderSortingTextField():void {
    ...
    var queryChunkFieldCfg:queryChunkField = new queryChunkField();
    queryChunkFieldCfg.bindTo = bindTo;
    queryChunkFieldCfg.defaultExpertExpression = sortingDefaultLocal;
    queryChunkFieldCfg.remoteExpertExpression = getExpertRemote(sortingRemote, sortingPropertyName);
    queryChunkFieldCfg.fieldLabel = QueryEditor_properties.INSTANCE.DCQE_label_sorting_search_query;
    add(queryChunkFieldCfg);
