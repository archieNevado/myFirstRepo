...
public class ApplicableConditionsBase extends Container {
  ...
  private function renderGroupedConditions():void {
    ...
    for(group in groupedConditions) {
      ...
      var panelCfg:ext.config.panel = new panel();
      panelCfg.title = getGroupTitle(group);
      panelCfg.cls = "applicable-condition-group";
      panelCfg.bodyCssClass = "applicable-condition-group-body";
      groupPanel = ComponentMgr.create(panelCfg) as Panel;
      for(var i:int=0; i<conditionsGroup.length; i++) {
        condition = conditionsGroup[i];
        var conditionDroppableCfg:conditionDroppable = new conditionDroppable();
        conditionDroppableCfg.text = ContentQueryEditorBase.getConditionTitle(condition.propertyName);
        conditionDroppableCfg.conditionEditorXtype = condition.xtype;
        groupPanel.add(conditionDroppableCfg);
      }
