...
//    fqValueExpression = getContentQueryEditor().getFilterQuery();
  ...
  public function createConditionEditor(conditionEditorConfig:conditionEditorBase):void {
    ...
    var newLabelConfig:label = new label();
    newLabelConfig.cls = "x-form-item-label";
    newLabelConfig.text = QueryEditor_properties.INSTANCE.DCQE_condition_target_title_AND;
    // The label will pushed towards the drop target when an editor is inserted in the same place.
    getAppliedConditionsContainer().insert(insertPosition, newLabelConfig);
    getAppliedConditionsContainer().insert(insertPosition, conditionEditorConfig);
    this.doLayout();
