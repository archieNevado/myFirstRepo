...
public class OpenUploadDialogActionBase extends ContentAction {
   ...
   * @param config
   */
  public function OpenUploadDialogActionBase(config:OpenUploadDialogAction = null) {
    super(config);
    if (!config.handler) {
  ...
  private static function getEditorialPathFromStudioConfiguration():Content {
    return StudioConfigurationUtil.getConfiguration("Content Creation", "paths.editorial");
  }
