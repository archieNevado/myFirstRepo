...
public class UploadProgressPanelBase extends Container {
  ...
  private var ignoredXliffResultCodes:Array;

  public function UploadProgressPanelBase(config:uploadProgressPanel = null) {
    super(config);
    this.settings = config.settings;
    this.file = config.file;
    this.folder = config.folder;
    this.callback = config.callback;
    this.ignoredXliffResultCodes = config.ignoredXliffResultCodes;
  }
  ...
  private function xliffUploaded(xliffBulkOperationResult:XliffBulkOperationResult):void {
    ...
    if (filteredResults.length > 0) {
      var xliffImportResultWindowCfg:xliffImportResultWindow = new xliffImportResultWindow();
      ...
      ComponentMgr.create(xliffImportResultWindowCfg).show();
    }
