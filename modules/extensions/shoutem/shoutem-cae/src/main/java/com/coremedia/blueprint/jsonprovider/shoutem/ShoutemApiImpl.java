...
public class ShoutemApiImpl implements ShoutemApi {
  ...
  private String isCommentable(ShoutemApiCredentials credentials) {
    ElasticSocialConfiguration elasticSocialPageAspect = elasticSocialPlugin(credentials.getSite());
    if (elasticSocialPageAspect.isCommentingEnabled()) {
  ...
  private String isLikeable(ShoutemApiCredentials credentials) {
    ElasticSocialConfiguration elasticSocialPageAspect = elasticSocialPlugin(credentials.getSite());
    if (elasticSocialPageAspect.isLikeEnabled()) {
      return ShoutemApi.VALUE_YES;
    }
    return ShoutemApi.VALUE_NO;
  }
  ...
  private boolean isPostModeration(ShoutemApiCredentials credentials) {
    ElasticSocialConfiguration elasticSocialPageAspect = elasticSocialPlugin(credentials.getSite());
    return elasticSocialPageAspect.getUserModerationType() == ModerationType.POST_MODERATION;
  }
  ...
  private CMNavigation getSiteNavigation(String site) {
    ContentRootNavigationsBySegmentCacheKey cacheKey = new ContentRootNavigationsBySegmentCacheKey(sitesService, urlPathFormattingHelper);
  ...
  ElasticSocialConfiguration elasticSocialPlugin(String site) {
    CMNavigation navigation = getSiteNavigation(site);
    return elasticSocialPlugin.getElasticSocialConfiguration(navigation);
  }
}
