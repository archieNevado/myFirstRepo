...
public class OpenTopicPagesEditorAction extends Action {
  ...
  public function OpenTopicPagesEditorAction(config:openTopicPagesEditorAction = null) {
  ...
  private static function updateTooltip(enabled:Boolean):void {
    var msg:String = Editor_properties.INSTANCE.Button_disabled_insufficient_privileges;

    var preferredSiteId:String = editorContext.getSitesService().getPreferredSiteId();
    if (!preferredSiteId) {
      msg = TopicPages_properties.INSTANCE.topic_pages_button_no_preferred_site_tooltip;
    }
    ...
    TopicsHelper.loadSettings(function (settingsRemoteBean:Bean):void {
      ...
      if (!topicPageChannel) {
        msg = TopicPages_properties.INSTANCE.topic_pages_button_no_topic_page_settings_tooltip;
        setButtonTooltip(msg);
      }
    });

    if (enabled) {
      msg = TopicPages_properties.INSTANCE.topic_pages_button_tooltip;
    }
