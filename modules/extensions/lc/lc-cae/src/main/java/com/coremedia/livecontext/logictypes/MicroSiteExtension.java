...
public class MicroSiteExtension extends ExtensionBase {
  ...
  public boolean isMicroSite(CMObject cmObject) {
    return (cmObject instanceof CMChannel) && (!(cmObject instanceof CMExternalChannel)) && renderCommerceMicroSiteLinks((CMChannel)cmObject);
  }
    ...
    try {
      if (renderCommerceMicroSiteLinks(channel)) {
        ...
        if (storeContext != null) {
          String seoSegment = externalSeoSegmentBuilder.asSeoSegment(channel, channel);
          String contentURLKeywordForMicroSite = getContentURLKeywordForMicroSite(channel);
          return buildCommerceLinkFor(null, contentURLKeywordForMicroSite + "/" + seoSegment , Collections.EMPTY_MAP);
        }
      }
    } catch (Exception e) {
