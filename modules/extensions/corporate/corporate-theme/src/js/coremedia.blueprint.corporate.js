...
coremedia.blueprint.corporate = function (module) {
      ...
      if ($container.length) {
        ...
        $container.each( function() {
          ...
          if ($this.hasClass("cm-superhero--video")) {
            coremedia.blueprint.logger.log("Superhero Teaser has video");
            var superHeroVideo = $this.find(".cm-superhero__video");
            var superHeroVideoPlayerId = superHeroVideo.attr("id");

            //youtube
            if (superHeroVideo.hasClass("cm-video--youtube")) {
              coremedia.blueprint.logger.log("Muting Superhero Teaser with Youtube video");
              ...
              coremedia.blueprint.logger.log("Muting Superhero Teaser with Vimeo video");
  ...
  module.affix = function () {
    ...
    if ($(window).width() > 1024) {
      var $mainContainer = $('#cm-main');
      if ($mainContainer.length) {
        var compareMain = true;
      }
      ...
      $('.cm-sticky').each(function () {
        ...
        if ($this.outerHeight() < $(window).height()) {
          if (compareMain && $this.outerHeight() > $mainContainer.outerHeight()) {
            coremedia.blueprint.logger.log(".cm-sticky is taller than main");
            return false;
          }
...
coremedia.blueprint.$(function () {
  ...
  if (deviceRatio > 1 && $("[data-cm-retina-image]").length > 0) {
    coremedia.blueprint.logger.log("Retina Images enabled. The DevicePixelRatio is: " + deviceRatio);
  }

  // initially load 360 spinner
  $(".cm-spinner__canvas").threeSixtySpinner();
  ...
  try {
    ...
    coremedia.blueprint.logger.log(err.message);
  }

  // showing videos (and more) in responsive popups
