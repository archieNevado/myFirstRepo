...
public class CommerceCacheInvalidationPropagatorImplTest {
  ...
  @Before
  public void setup(){
    testling = new CommerceCacheInvalidationPropagatorImpl(cache);
  }
  ...
  public void testInvalidateClearAll(){
    List<CommerceCacheInvalidation> invalidations = new ArrayList<>();
    CommerceCacheInvalidationImpl event = new CommerceCacheInvalidationImpl();
    event.setContentType(CommerceCacheInvalidation.EVENT_CLEAR_ALL_EVENT_ID);
    event.setTechId("0");
    invalidations.add(event);
