 ...
 */
class CommerceCacheInvalidationPropagatorImpl implements CommerceCacheInvalidationPropagator {
  private static final Logger LOG = LoggerFactory.getLogger(CommerceCacheInvalidationPropagatorImpl.class);

  private final Cache cache;

  CommerceCacheInvalidationPropagatorImpl(Cache cache) {
    this.cache = cache;
  }
  ...
  public void invalidate(List<CommerceCacheInvalidation> invalidations) {
    for (CommerceCacheInvalidation invalidation : invalidations) {
      ...
      try {
        if (invalidation.getContentType().equals(CommerceCacheInvalidation.EVENT_CLEAR_ALL_EVENT_ID)) {
