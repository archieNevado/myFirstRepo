...
class WcCacheWrapperService extends AbstractWcWrapperService {
  ...
  Map<String, Object> getCacheInvalidations(long lastExecutionTimeStamp, @Nonnull StoreContext storeContext) throws CommerceException {
    ...
    Map<String, Object> wcCommerceCacheInvalidations = getRestConnector().callService(GET_CACHE_INVALIDATION, asList(String.valueOf(lastExecutionTimeStamp), String.valueOf(MAX_POLL_WAIT), String.valueOf(CHUNK_SIZE)), Collections.<String, String[]>emptyMap(), null, storeContext, null);
  ...
  long getLatestTimestamp(@Nonnull StoreContext storeContext) throws CommerceException {
    long result = -1;
    Map map = getRestConnector().callService(GET_LATEST_TIMESTAMP, Collections.singletonList(String.valueOf(MAX_POLL_WAIT)), Collections.<String, String[]>emptyMap(), null, storeContext, null);
    if (map == null) {
      //404 and 204
