...
<div class="cm-teaser cm-teaser--category"<@cm.metadata metadata![] />>
  <@bp.optionalLink href="${cm.getLink(self)}">
    <div class="cm-teaser__content cm-aspect-ratio-box">
        <img class="cm-aspect-ratio-box__content" <@cm.dataAttribute name="data-cm-non-adaptive-content" data={"overflow": true} /> src="${(self.category.thumbnailUrl)!""}" alt="${(self.category.name)!""}" />
    </div>
