...
public class CssTransformer extends AbstractTransformer {
  ...
  private void writeLineAsP(String line) throws IOException {
    write("<p>");
    write(line);
    write("</p>");
  }
  ...
  private String urlsToXlinks(String line, URI systemId) throws URISyntaxException {
    ...
    while (matcher.find()) {
      ...
      String replacement = urlReplacement(systemId, uri, protocol, path);
  ...
  private String urlReplacement(URI systemId, String uri, String protocol, String path) throws URISyntaxException {
    if (protocol==null) {
      return toRichtextInternalLink(systemId, path);
    } else if ("data".equals(protocol)) {
      return toRichtextPlain(protocol, path);
    } else {
      return toRichtextHref(createUri(uri), uri);
    }
  }
