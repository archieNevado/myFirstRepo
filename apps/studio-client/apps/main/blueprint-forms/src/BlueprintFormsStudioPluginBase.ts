...
import EditorMainNavigationToolbar from "@coremedia/studio-client.main.editor-components/sdk/desktop/maintoolbar/EditorMainNavigationToolbar";
...
class BlueprintFormsStudioPluginBase extends StudioPlugin {
  ...
  #contentTypeVE: ValueExpression = null;
  ...
  static calculateQuickCreateFolder(): string {
    const libToggleBtn = as(Ext.getCmp(EditorMainNavigationToolbar.LIBRARY_BUTTON_ITEM_ID), Button);
    if (libToggleBtn) {
   ...
   */
  protected isValidCMLinkable(content: Content): boolean {
  ...
  protected getContentTypeVE(): ValueExpression {
    if (!this.#contentTypeVE) {
      this.#contentTypeVE = ValueExpressionFactory.createFromFunction((): string => {
