...
import HandleEmptySearchResultPlugin from "@coremedia/studio-client.main.editor-components/sdk/collectionview/HandleEmptySearchResultPlugin";
import HeaderToolbarUserMenu from "@coremedia/studio-client.main.editor-components/sdk/desktop/HeaderToolbarUserMenu";
  ...
  constructor(config: Config<ECommerceStudioPlugin> = null) {
    ...
    super(ConfigUtils.apply(Config(ECommerceStudioPlugin, {
      ...
      rules: [
        ...
        Config(HeaderToolbarUserMenu, {
          plugins: [
            Config(AddItemsPlugin, {
              items: [
                Config(DisplayField, {
                  margin: "0 4 0 4",
                  fieldLabel: ECommerceStudioPlugin_properties.HeaderToolbar_commerceStore_label,
                  labelAlign: "top",
                  labelSeparator: "",
                  ...Config<BEMMixin>({ bemElement: HeaderToolbarUserMenu.ELEMENT_FIELD }),
                  plugins: [
                    Config(BindPropertyPlugin, { bindTo: this$.getShopExpression() }),
                    Config(BindVisibilityPlugin, { bindTo: this$.getShopExpression() }),
                  ],
                }),
              ],
              after: [
                Config(Component, { itemId: HeaderToolbarUserMenu.SITE_SELECTOR_ITEM_ID }),
              ],
            }),
          ],
        }),

        Config(PreferenceWindow, {
          plugins: [
