...
class CatalogTreeModel implements CompoundChildTreeModel, LazyLoadingTreeModel {
  ...
  static #computeStoreText(): string {
    ...
    if (CatalogTreeModel.#getStore().getCurrentWorkspace()) {
      workspaceName = CatalogTreeModel.#getStore().getCurrentWorkspace().getName();
    }
    return CatalogTreeModel.#getStore().getName() + (workspaceName ? " - " + workspaceName : "");
