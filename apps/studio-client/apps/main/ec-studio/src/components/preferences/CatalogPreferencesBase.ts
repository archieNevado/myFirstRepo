...
class CatalogPreferencesBase extends Panel implements PreferencePanel {
  ...
  showCatalogValueExpression: ValueExpression = null;

  sortCategoriesByNameExpression: ValueExpression = null;
  ...
  protected getShowCatalogValueExpression(): ValueExpression {
    if (!this.showCatalogValueExpression) {
      const enabled: boolean = ValueExpressionFactory.create(CatalogPreferencesBase.PREFERENCE_SHOW_CATALOG_KEY, editorContext._.getPreferences()).getValue();
  ...
  protected getSortCategoriesByNameExpression(): ValueExpression {
    if (!this.sortCategoriesByNameExpression) {
      const enabled: string = ValueExpressionFactory.create(CatalogPreferencesBase.SORT_CATEGORIES_BY_NAME_KEY, editorContext._.getPreferences()).getValue();
