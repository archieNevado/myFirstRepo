...
class CatalogHelper {
  ...
  static readonly NO_WS: string = CatalogModel.NO_WS;
  ...
  static readonly COMMERCE_STRUCT_WORKSPACE: string = "workspace";
  ...
  getCatalogObject(catalogObjectId: string, contentExpression?: ValueExpression): CatalogObject {
    ...
    let workspaceId = this.getExtractedWorkspaceId();
    if (!workspaceId) {
      workspaceId = CatalogHelper.NO_WS;//No workspace selected
    }
    ...
    } else if (this.#isContractId(catalogObjectId)) {
  ...
  #isContractId(catalogObjectId: string): boolean {
  ...
  getActiveStoreExpression(): ValueExpression {
    ...
    this.#storeExpression = ValueExpressionFactory.createFromFunction(bind(this, this.getActiveStore));
    return this.#storeExpression;
  }

  getActiveStore(): Store {
    const siteId = editorContext._.getSitesService().getPreferredSiteId();
    return this.getValidatedStore(siteId);
  }
  ...
  getExtractedWorkspaceId(): string {
    const workspaceFullId: string = this.getCommerceWorkspaceExpression().getValue();
    if (!workspaceFullId) {
      return CatalogHelper.NO_WS;//No workspace selected;
    }
    return this.getExternalIdFromId(workspaceFullId);
  }

  getCommerceWorkspaceExpression(): ValueExpression {
    return ValueExpressionFactory.create(null, editorContext._.getPreferences()).
      extendBy(CatalogHelper.#PREFERENCES_COMMERCE_STRUCT).extendBy(CatalogHelper.COMMERCE_STRUCT_WORKSPACE);
  }
   ...
   */
  getValidatedStore(siteId: string): Store {
    if (siteId === undefined) {
      return undefined;
    }
    if (siteId === null) {
    ...
    const workspaceId = this.getExtractedWorkspaceId();
    const store = as(beanFactory._.getRemoteBean("livecontext/store/" + siteId + "/" + workspaceId), Store);
  ...
  getDisplayName(catalogObject: CatalogObject): string {
    const name: string = is(catalogObject, Category) ? (catalogObject as Category).getDisplayName() : this.getDecoratedName(catalogObject);
    return name;
  }
    ...
    return new Promise((resolve: AnyFunction): void => {
      ...
      localSettingsStructExpression.loadValue((): void => {
        ...
        cast(RemoteBean, localSettingsStruct).load((): void => {
          ...
          //avoid duplicates
          if (product && (commerceStruct.get(CatalogHelper.REFERENCES_LIST_NAME) as Array).indexOf(product.getId()) < 0) {
  ...
  static getCatalogObjectsExpression(contentExpression: ValueExpression,
    ...
    catalogObjectIdsExpression?: ValueExpression): ValueExpression {
    ...
    return ValueExpressionFactory.createFromFunction((): Array<any> => {
      let catalogObjects: Array<any> = idsExpression.getValue();
      if (!catalogObjects) {
        return undefined;
      }
      catalogObjects = catalogObjects.map((id: string): Bean => {
      ...
      catalogObjects = catalogObjects.filter((obj: any): boolean =>
        obj,
      );
