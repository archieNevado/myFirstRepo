...
class CreateFromTemplateDialogBase extends StudioDialog {
  ...
  protected nameValidator(callback: AnyFunction): void {
    ...
    let folder: Content = this.getModel().get(ProcessingData.FOLDER_PROPERTY);
    if (is(folder, Array)) {
      folder = folder[0];
    }
  ...
  protected contentBaseFolderValidator(folderChooserListView: FolderChooserListView): boolean {
    let folder: Content = this.getModel().get(CreateFromTemplateStudioPluginSettings_properties.editorial_folder_property);
    if (is(folder, Array)) {
      folder = folder[0];
    }
