...
class CommerceObjectsDisplayFieldBase extends DisplayField {
  ...
  getCommerceObjectsExpression(): ValueExpression {
    return ValueExpressionFactory.createFromFunction((): string => {
      ...
      const catalogObjects: Array<any> = CatalogHelper.getCatalogObjectsExpression(this.#getPersonaContentExpression(),
      ...
      const commerceObjectsAsString = catalogObjects.map((commerceObject: Bean): string => {
