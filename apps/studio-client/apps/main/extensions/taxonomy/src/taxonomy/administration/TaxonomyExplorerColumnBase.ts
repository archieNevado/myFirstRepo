...
class TaxonomyExplorerColumnBase extends GridPanel {
  ...
  #onKeyInput(grid: any, td: any, cellIndex: any, record: any, tr: any, rowIndex: any, e: any, eOpts: any): boolean {
  ...
  protected selectionChanged(): void {
    const selections: Array<any> = (this.getSelectionModel() as RowSelectionModel).getSelection();
      ...
      if (this.#activeNode) {
        for (let i = 0; i < this.getStore().getCount(); i++) {
          ...
          if (nodeRecord.data.ref === this.#activeNode.getRef()) {
            (this.getSelectionModel() as RowSelectionModel).select([nodeRecord], false, true);
            this.getView().focusRow(i);
          }
        }
      } else {
        (this.getSelectionModel() as RowSelectionModel).deselectRange(0, this.getStore().getCount() - 1);
      }
  ...
  #updateLoadStatus(text: string): void {
    (this.getView() as unknown)["emptyText"] = text;
  ...
  protected pointerColRenderer(value: any, metaData: any, record: Model): string {
    ...
    if (!leaf) {
      return "<span class=\"" + CoreIcons_properties.arrow_right + "\"></span>";
    }
    return "";
  }
