...
class TaxonomyLinkListPropertyFieldBase extends FieldContainer {
  ...
  #searchResultChanged(): void {
    ...
    if (selection) {
      ...
      const propertyValueExpression = ValueExpressionFactory.create("properties." + this.propertyName, this.bindTo.getValue());
      const taxonomies: Array<any> = propertyValueExpression.getValue();
