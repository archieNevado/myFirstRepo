...
import AccessControlUtil from "@coremedia/studio-client.main.editor-components/sdk/util/AccessControlUtil";
...
class OpenTaxonomyChooserActionBase extends DependencyTrackedAction {
  ...
  protected override calculateDisabled(): boolean {
    if (this.bindTo && is(this.bindTo.getValue(), Content) && this.forceReadOnlyValueExpression) {
      return (this.bindTo.getValue() as Content).isCheckedOutByOther() || AccessControlUtil.isReadOnly(this.bindTo.getValue()) || this.forceReadOnlyValueExpression.getValue();
    }
    return false;
  }
