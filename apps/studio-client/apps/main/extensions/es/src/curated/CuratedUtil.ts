...
class CuratedUtil {
  ...
  static postCreateArticleFromComments(content: Content): void {
    const remoteServiceMethod = new RemoteServiceMethod((ModerationImpl.getInstance() as ModerationImpl).getTenantAwareESUriPrefix() + "/curate/comments", "POST");
  ...
  static postCreateGalleryFromComments(content: Content): void {
    const remoteServiceMethod = new RemoteServiceMethod((ModerationImpl.getInstance() as ModerationImpl).getTenantAwareESUriPrefix() + "/curate/images", "POST");
    const params: any = CuratedUtil.#makeRequestParameters(content);
    remoteServiceMethod.request(params, CuratedUtil.#openOnSuccessfullyCreatedArticle, null);
  }
  ...
  static #getCommentIds(): string {
    ...
    for (let i: number = 0; i < commentArray.length; i++) {
      const commentId = (commentArray[i] as ModeratedItem).getTargetId();
