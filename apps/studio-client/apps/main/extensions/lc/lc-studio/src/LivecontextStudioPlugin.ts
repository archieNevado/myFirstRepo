...
import CommerceWorkspaceSelector from "./components/CommerceWorkspaceSelector";
...
class LivecontextStudioPlugin extends LivecontextStudioPluginBase {
  ...
  static override readonly EXTERNAL_ID_PROPERTY: string = "externalId";
      ...
      rules: [
        ...
        Config(HeaderToolbarUserMenu, {
          plugins: [
            Config(AddItemsPlugin, {
              items: [
                Config(CommerceWorkspaceSelector, <Config<CommerceWorkspaceSelector>>Config<BEMMixin>({ bemElement: HeaderToolbarUserMenu.ELEMENT_FIELD })),
              ],
              after: [
        ...
        Config(PreviewPanel, {
          plugins: [
            Config(BindPlugin, {
              bindTo: CatalogHelper.getInstance().getCommerceWorkspaceExpression(),
              boundValueChanged: LivecontextStudioPluginBase.reloadPreview,
            }),
          ],
        }),
