...
class CreateCatalogObjectDocumentActionBase extends LiveContextCatalogObjectAction {
  ...
  constructor(config: Config<CreateCatalogObjectDocumentAction> = null) {
    ...
    const this$ = this;
    super(cast(CreateCatalogObjectDocumentAction, ActionConfigUtil.extendConfiguration(resourceManager.getResourceBundle(null, LivecontextStudioPlugin_properties).content, config, config.actionName, { handler: bind(this$, this$.myHandler) })));
