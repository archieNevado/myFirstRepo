...
import StructType from "@coremedia/studio-client.cap-rest-client/struct/StructType";
...
import { as, asConfig, bind, cast, is } from "@jangaroo/runtime";
...
class LivecontextStudioPluginBase extends StudioPlugin {
  ...
  override init(editorContext: IEditorContext): void {
    ...
    editorContext.registerPreviewUrlTransformer((uri: PreviewURI, callback: AnyFunction): void => {
      const workspaceId = CatalogHelper.getInstance().getExtractedWorkspaceId();
     ...
     */
    editorContext.registerContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_MARKETING_SPOT, LivecontextStudioPluginBase.#initMarketingSpot);
    editorContext.registerContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_PRODUCT_LIST, LivecontextStudioPluginBase.#initProductList);
    editorContext.registerContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_PRODUCT_TEASER, LivecontextStudioPluginBase.#initProductTeaser);
    editorContext.registerContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_CHANNEL, LivecontextStudioPluginBase.#initExternalChannel);
    editorContext.registerContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_PRODUCT, LivecontextStudioPluginBase.#initExternalProduct);
    editorContext.registerContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_PAGE, LivecontextStudioPluginBase.#initExternalPage);
     ...
     */
    editorContext.extendContentInitializer(LivecontextStudioPlugin.CONTENT_TYPE_IMAGE_MAP, LivecontextStudioPluginBase.#extendImageMap);

    /**
     * apply the marketing spot link field to CMMarketingSpot quick create dialog
     */
    QuickCreate.addQuickCreateDialogProperty(LivecontextStudioPlugin.CONTENT_TYPE_MARKETING_SPOT, LivecontextStudioPluginBase.EXTERNAL_ID_PROPERTY,
      (data: ProcessingData, properties: any): Component => {
        const config = cast(CatalogLinkPropertyField, properties);
        config.dropAreaHandler = bind(CatalogHelper.getInstance(), CatalogHelper.getInstance().openMarketingSpots);
     ...
     */
    QuickCreate.addQuickCreateDialogProperty(LivecontextStudioPlugin.CONTENT_TYPE_PRODUCT_TEASER, LivecontextStudioPluginBase.EXTERNAL_ID_PROPERTY,
      (data: ProcessingData, properties: any): Component => {
        const config = cast(CatalogLinkPropertyField, properties);
        config.maxCardinality = 1;
        config.replaceOnDrop = true;
     ...
     */
    cast(EditorContextImpl, editorContext).registerTabTooltipHandler(LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_CHANNEL, LivecontextStudioPluginBase.#computeCatalogToolTip);
    cast(EditorContextImpl, editorContext).registerTabTooltipHandler(LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_PRODUCT, LivecontextStudioPluginBase.#computeCatalogToolTip);
    cast(EditorContextImpl, editorContext).registerTabTooltipHandler(LivecontextStudioPlugin.CONTENT_TYPE_PRODUCT_TEASER, LivecontextStudioPluginBase.#computeCatalogToolTip);
    ...
    MetaStyleService.setDefaultStyleForMetaData(MetaStyleService.STYLE_GROUP_YELLOW, [LivecontextStudioPlugin.CONTENT_TYPE_PRODUCT_LIST]);
    MetaStyleService.setDefaultStyleForMetaData(MetaStyleService.STYLE_GROUP_PURPLE, [
      LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_PAGE, LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_CHANNEL,
    ]);
    MetaStyleService.setDefaultStyleForMetaData(MetaStyleService.STYLE_GROUP_TURQUOISE, [
      LivecontextStudioPlugin.CONTENT_TYPE_PRODUCT_TEASER, LivecontextStudioPlugin.CONTENT_TYPE_MARKETING_SPOT,
      LivecontextStudioPlugin.CONTENT_TYPE_EXTERNAL_PRODUCT,
    ]);
  }
