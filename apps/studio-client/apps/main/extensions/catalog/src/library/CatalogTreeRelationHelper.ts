...
class CatalogTreeRelationHelper {
  ...
  static addCategoryChild(parentCategory: Content, child: Content): Content {
    ...
    if (child.getType().isSubtypeOf(CatalogTreeRelationHelper.CONTENT_TYPE_CATEGORY)) {
      ...
      const parentChildren = (parentCategory.getProperties().get(CatalogTreeRelationHelper.PROPERTY_CHILDREN) as Array).slice();
      ...
      const contexts = (child.getProperties().get(CatalogTreeRelationHelper.PROPERTY_CONTEXTS) as Array).slice();
