...
service_name = 'headless-server-live'
...
node.default_unless['blueprint']['apps'][service_name]['application.properties']['repository.url'] = 'http://localhost:40280/ior'
node.default_unless['blueprint']['apps'][service_name]['application.properties']['solr.url'] = 'http://localhost:40080/solr'
node.default_unless['blueprint']['apps'][service_name]['application.properties']['caas.solr.collection'] = 'live'
node.default_unless['blueprint']['apps'][service_name]['application.properties']['spring.application.name'] = 'headless-server-live'
...
node.default_unless['blueprint']['apps'][service_name]['application.properties']['caas.remote.baseurl'] = "http://#{node['blueprint']['hostname']}:42180/blueprint/servlet/internal/service/url"
...
node.default_unless['blueprint']['apps'][service_name]['application.properties']['commerce.hub.data.customEntityParams.environment'] = 'live'
...
if spring_boot_default(service_name, 'debug')
  java_opts_hash['debug'] = '-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:41206'
end
...
spring_boot_application service_name do
  ...
  post_start_wait_url "http://localhost:41281/actuator/health"
  ...
  jmx_remote_registry_port 41299
  jmx_remote_server_port 41298
