...
node.default_unless['blueprint']['apps'][service_name]['application.properties']['server.port'] = 40680
node.default_unless['blueprint']['apps'][service_name]['application.properties']['management.server.port'] = 40681
...
if spring_boot_default(service_name, 'debug')
  java_opts_hash['debug'] = '-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:40606'
end
...
spring_boot_application service_name do
  ...
  post_start_wait_url "http://localhost:40681/actuator/health"
  ...
  jmx_remote_registry_port 40699
  jmx_remote_server_port 40698
