...
node.default_unless['blueprint']['apps'][service_name]['application.properties']['mongodb.client-uri'] = 'mongodb://coremedia:coremedia@localhost:27017'
...
# the old service key was studio, thats why we import here the legacy config from
application_config_hash = Chef::Mixin::DeepMerge.hash_only_merge!(application_config_hash, node['blueprint']['webapps']['studio']['application.properties']) if node.deep_fetch('blueprint', 'webapps', 'studio', 'application.properties')
...
directory node['blueprint']['apps'][service_name]['application.properties']['themeImporter.apiKeyStore.basePath'] do
  owner service_name
  group service_group
  recursive true
  mode 0700
...
if spring_boot_default(service_name, 'debug')
  java_opts_hash['debug'] = '-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:41006'
end
...
spring_boot_application service_name do
  ...
  post_start_wait_url "http://localhost:41081/actuator/health"
  ...
  jmx_remote_registry_port 41099
  jmx_remote_server_port 41098
