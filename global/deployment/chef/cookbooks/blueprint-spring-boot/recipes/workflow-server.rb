...
node.default_unless['blueprint']['apps'][service_name]['application.properties']['cap.client.server.ior.url'] = 'http://localhost:40180/ior'
...
boot_opts_config_hash = Chef::Mixin::DeepMerge.hash_only_merge!(boot_opts_config_hash, node['blueprint']['apps'][service_name]['boot_opts']) if node.deep_fetch('blueprint', 'apps', service_name, 'boot_opts')
...
if spring_boot_default(service_name, 'debug')
  java_opts_hash['debug'] = '-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:40306'
end
...
spring_boot_application service_name do
  ...
  post_start_wait_url "http://localhost:40380/ior"
  ...
  jmx_remote_registry_port 40399
  jmx_remote_server_port 40398
