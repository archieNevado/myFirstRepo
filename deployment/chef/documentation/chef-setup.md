...
`cookbooks` folder. In general, you should not modify files in these cookbooks, they will be updated with each release
...
to third-party cookbooks are mostly using the pessimistic `~>` version constraint, therefore updating thirdparty cookbooks within the
constrained version range, can be done easily using berkshelf. 

If you want to manage third-party cookbooks yourself, you simply need to remove the cookbook from the `thirdparty-cookbooks` 
...
There are cookbooks CoreMedia provides, which have library character and do not have any dependencies to CoreMedia Blueprint.
Those cookbooks must be treated like third-party cookbooks, except they are not available from the public supermarket. The sources
of these cookbooks are available as git repositories, which you can fork and change. We highly recommend not to do this except to provide
pull requests our origin repo. We will gladly review, apply your changes and release a new version, which is then immediately available as a 
...
therefore are immanent subject to change in your project. Therefore, you should not treat these cookbooks as libraries where easy migration
is your top priority. These cookbooks are yours, you should change them to fit your projects needs.

Some of these cookbooks are `wrapper` cookbooks. They just wrap a third-party cookbook by setting attributes and include the original
...
Other cookbooks are `application` cookbooks. They use resources or definitions provided by third-party cookbooks to create infrastructure. 
...
CoreMedia Blueprint applications tomcats and therefore also contain most of the applications configuration properties.

* The `blueprint-proxy` cookbook contains recipes to install an configure Apache HTTPD server and configure the virtual hosts for CoreMedia Blueprint including
...
There are four environments, `kitchen`, `development`, `staging` and `production`. Environments should not contain attributes as well, as they are
...
* A `Gemfile` to set up the development environment with [bundler](http://bundler.io/).
...
1. Start developing in any blueprint cookbook and change the directory to that cookbook.
...
Before committing, you should regenerate the documentation using `bundle exec rake doc`. For a documentation on how to use the annotations, see [knife-cookbooc-doc](https://github.com/realityforge/knife-cookbook-doc).

CI Tooling
---------------

To setup a cookbook CI, we recommend to use Jenkins with test-kitchen and a driver and infrastructure combination, that allows to start each ci run
...
In case your Jenkins slaves are Chef managed as well, you need to install berkshelf and the kitchen gems securely. For this purpose, we provide you with the 
