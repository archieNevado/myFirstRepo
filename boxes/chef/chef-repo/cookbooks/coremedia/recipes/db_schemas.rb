if node["coremedia"]["db"]["type"] == "postgresql"
  log "You need to include the recipe 'mysql::client' to run this recipe" do
    level :warn
    not_if { node.recipe?('mysql::client') }
  end
  ...
  log "You need to include the recipe 'postgresql::client' to run this recipe" do
    level :warn
    not_if { node.recipe?('mysql::client') }
  end
  node["coremedia"]["db"]["schemas"].each do |schema|
