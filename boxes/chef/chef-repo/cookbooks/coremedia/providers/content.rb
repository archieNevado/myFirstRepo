...
action :publishall do
  Chef::Log.info("#{new_resource.name} - publish all content")
  coremedia_probedog 'check-cms-before-publish' do
    tool_name 'cm7-cms-tools'
    action :check
    probe 'ProbeContentServerOnline'
  ...
  execute "#{new_resource.name}_check_in_all" do
    user node["coremedia"]["user"] unless platform_family? 'windows'
    command "#{new_resource.cms_tools}#{cm_exec} bulkpublish #{login_params} -c"
    retries 0
    sensitive true
    timeout new_resource.timeout
  end

  execute "#{new_resource.name}_publishall" do
    user node["coremedia"]["user"] unless platform_family? 'windows'
    command "#{new_resource.cms_tools}#{cm_exec} publishall -a -cq \"#{node["coremedia"]["publishall"]["contentquery"]}\" http://localhost:41080/coremedia/ior #{new_resource.username} #{new_resource.password} http://localhost:42080/coremedia/ior admin admin"
    retries 0
    sensitive true
    timeout new_resource.timeout
  end
